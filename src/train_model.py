...
with mlflow.start_run(run_name="LinearRegression") as run_lr:
    ...
    mlflow.sklearn.log_model(lr, "model", registered_model_name="LinearRegression_California")

    print(f"LinearRegression - RMSE: {rmse:.2f}, R2: {r2:.2f}")
    results["LinearRegression"] = {"rmse": rmse, "model": lr, "run_id": run_lr.info.run_id}
...
with mlflow.start_run(run_name="DecisionTree") as run_dt:
    ...
    mlflow.sklearn.log_model(dt, "model", registered_model_name="DecisionTree_California")

    print(f"DecisionTree - RMSE: {rmse:.2f}, R2: {r2:.2f}")
    results["DecisionTree"] = {"rmse": rmse, "model": dt, "run_id": run_dt.info.run_id}

# Pick best model based on lowest RMSE
best_model_name = min(results, key=lambda k: results[k]["rmse"])
print(f"\n✅ Best Model: {best_model_name} (Run ID: {results[best_model_name]['run_id']})")
